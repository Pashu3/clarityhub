...
export default function Documentation({
  ...
  const noContentMessage = (
    <div>
      <FileCode size={48} className="text-slate-400 mx-auto mb-4" />
      <p className="font-medium text-lg mb-1 text-slate-700 dark:text-slate-300">No documentation found</p>
      <p className="text-slate-500 dark:text-slate-400">Create technical documentation for your product</p>
    </div>
  );
                ...
                <tr className="border-b border-slate-100 dark:border-slate-700">
                  <th className="px-6 py-3">
                    <div className="flex items-center">
                      <span className="ml-3 text-xs font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">Title</span>
                    </div>
                  </th>
                        ...
                        >
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="ml-3">
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                  {item.category} • {item.readTime} read
                                </p>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex items-center text-sm text-slate-700 dark:text-slate-300">
                              <span className="p-1.5 rounded-md bg-slate-100 dark:bg-slate-700 mr-2">
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-10 text-center text-slate-500 dark:text-slate-400">
                        {searchQuery || typeFilter || statusFilter ? (
                          <div>
                            <p className="font-medium mb-1">No documentation matches your filters</p>
            ...
            paginatedContent.map(item => (
              ...
              >
                <div className="relative">
                  <div className={`h-40 bg-slate-200 dark:bg-slate-700 flex items-center justify-center ${!item.image ? 'bg-gradient-to-r from-purple-500/20 to-blue-500/20 dark:from-purple-900/30 dark:to-blue-900/30' : ''}`}>
                    {item.image ? (
                      <img src={item.image} alt={item.title} className="w-full h-full object-cover" />
                    ) : (
                      <FileCode size={48} className="text-slate-400 dark:text-slate-500" />
                    )}
                  </div>
                  <div className="absolute top-3 right-3 flex space-x-2">
                    <div className={`px-2 py-1 rounded text-xs font-medium ${getStatusBadgeStyles(item.status)}`}>
                      {item.status}
                    </div>
                  </div>
                ...
                <div className="p-4">
                  ...
                  <h3 className="font-medium text-lg text-slate-900 dark:text-white mb-2">{item.title}</h3>
            ...
            >
              {searchQuery || typeFilter || statusFilter ? (
                <div>
                  ...
                  <p className="font-medium text-lg mb-1 text-slate-700 dark:text-slate-300">No documentation matches your filters</p>
