...
export default function ArticlesAndGuides({
  paginatedContent,
  searchQuery,
  typeFilter,
  statusFilter,
  ...
  containerVariants,
  itemVariants
}: ArticlesAndGuidesProps) {
  
  const noContentMessage = (
    <div>
      <BookOpen size={48} className="text-slate-400 mx-auto mb-4" />
      <p className="font-medium text-lg mb-1 text-slate-700 dark:text-slate-300">No articles or guides found</p>
      <p className="text-slate-500 dark:text-slate-400">Create educational content for your users</p>
    </div>
  );
                  ...
                  <div className={`h-40 bg-slate-200 dark:bg-slate-700 flex items-center justify-center ${!item.image ? 'bg-gradient-to-r from-purple-500/20 to-blue-500/20 dark:from-purple-900/30 dark:to-blue-900/30' : ''}`}>
                    {item.image ? (
                      ...
                      item.type === 'Guide' ? 
                        ...
                        <Video size={48} className="text-slate-400 dark:text-slate-500" />
                    )}
                  </div>
            ...
            >
              {searchQuery || typeFilter || statusFilter ? (
                <div>
                  ...
                  <p className="font-medium text-lg mb-1 text-slate-700 dark:text-slate-300">No articles match your filters</p>
